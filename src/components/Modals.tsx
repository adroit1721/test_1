...
import { loginWithPin } from '../utils/apiClient';
...
export const AdminLoginModal: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Authenticate with backend for JWT session
    const res = await loginWithPin(enteredPin);
    const storedPin = localStorage.getItem('ngdc_admin_pin') || localStorage.getItem('ngdc_admin_service_pin') || '1721';

    if (res.success || enteredPin === storedPin.trim()) {
      setStatus('success');
      ...
      setErrorMsg(res.error || 'Invalid Service PIN. Default PIN is 1721.');
    }
  };
