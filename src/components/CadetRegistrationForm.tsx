...
export const CadetRegistrationForm: React.FC<CadetRegistrationFormProps> = ({
  ...
  const { cadetRegister, updateCadetUser, requestProfileUpdate } = useAdminData();
  ...
  useEffect(() => {
    if (editingCadet) {
      const isEx =
        editingCadet.cadetType === 'Ex-cadet' ||
        editingCadet.category === 'Ex-cadets' ||
        editingCadet.category === 'Ex-cadet Platoon' ||
        editingCadet.platoon === 'Ex-cadets Alumni';

      const formatAddr = (addr: any) => {
        if (!addr) return '';
        if (typeof addr === 'string') return addr;
        if (typeof addr === 'object') {
          return addr.address || [addr.division, addr.district, addr.upazila, addr.union, addr.village].filter(Boolean).join(', ');
        }
        return String(addr);
      };
      ...
      if (isEx) {
        ...
        setExForm({
          avatarUrl: editingCadet.avatarUrl || editingCadet.photoUrl || editingCadet.passportPhotoUrl || '',
          rank: editingCadet.rank || 'Cadet Under Officer (CUO)',
          batch: editingCadet.batch || '',
          cadetNo: editingCadet.cadetNo || editingCadet.cadetNumber || editingCadet.collegeId || '',
          password: editingCadet.password || '',
          name: editingCadet.name || editingCadet.fullName || editingCadet.nameEnglish || '',
          nameBangla: editingCadet.nameBangla || editingCadet.bnFullname || '',
          currentJob: editingCadet.currentJob || '',
          dob: editingCadet.dob || editingCadet.dateOfBirth || '',
          bloodGroup: editingCadet.bloodGroup || 'B+',
          gender: (editingCadet.gender as any) || 'Male',
          religion: editingCadet.religion || 'Islam',
          presentAddress: formatAddr(editingCadet.presentAddress),
          permanentAddress: formatAddr(editingCadet.permanentAddress),
          phone: editingCadet.phone || editingCadet.mobile || editingCadet.phoneSelf || '',
        ...
        let platVal: 'Male Platoon' | 'Female Platoon' | 'Band Platoon' = 'Male Platoon';
        const rawP = (editingCadet.category || editingCadet.platoon || '').toString();
        if (rawP.includes('Female')) platVal = 'Female Platoon';
        else if (rawP.includes('Band')) platVal = 'Band Platoon';
        ...
        setServingForm({
          ...
          name: editingCadet.name || editingCadet.fullName || editingCadet.nameEnglish || '',
          nameBangla: editingCadet.nameBangla || editingCadet.bnFullname || '',
          fatherName: editingCadet.fatherName || editingCadet.fatherNameEnglish || '',
          fatherNameBangla: editingCadet.fatherNameBangla || '',
          motherName: editingCadet.motherName || editingCadet.motherNameEnglish || '',
          motherNameBangla: editingCadet.motherNameBangla || '',
          dob: editingCadet.dob || editingCadet.dateOfBirth || '',
          bloodGroup: editingCadet.bloodGroup || 'B+',
          gender: (editingCadet.gender as any) || 'Male',
          religion: editingCadet.religion || 'Islam',
          className: editingCadet.className || editingCadet.studentClass || '11th',
          department: editingCadet.department || '',
          presentAddress: formatAddr(editingCadet.presentAddress),
          permanentAddress: formatAddr(editingCadet.permanentAddress),
          phone: editingCadet.phone || editingCadet.mobile || editingCadet.phoneSelf || '',
          guardianPhone: editingCadet.guardianPhone || editingCadet.phoneGuardian || '',
          email: editingCadet.email || '',
          additionalSkills: editingCadet.additionalSkills || '',
          achievements: editingCadet.achievements || '',
        });
  ...
  const handleServingSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMsg(null);

    const effectiveCadetNo = (servingForm.cadetNo || editingCadet?.cadetNo || '').trim().toUpperCase();
    const effectivePassword = servingForm.password.trim() || editingCadet?.password || '';
    const effectiveAvatar = servingForm.avatarUrl.trim() || editingCadet?.avatarUrl || '';
    ...
    if (!isProfileUpdateMode && !effectivePassword) {
    ...
    if (!isProfileUpdateMode && !effectiveAvatar) {
    ...
    const cadetPayload: Partial<CadetUserAccount> = {
      ...
      collegeId: effectiveCadetNo,
    };

    if (editingCadet) {
      if (isProfileUpdateMode) {
        const res = await requestProfileUpdate(editingCadet.cadetNo, cadetPayload);
        ...
        if (res.success) {
          ...
          setErrorMsg(res.message || 'Failed to submit profile update request.');
        }
        return;
      }
  ...
  const handleExSubmit = async (e: React.FormEvent) => {
    ...
    const effectiveCadetNo = (exForm.cadetNo || editingCadet?.cadetNo || '').trim().toUpperCase();
    const effectivePassword = exForm.password.trim() || editingCadet?.password || '';
    const effectiveAvatar = exForm.avatarUrl.trim() || editingCadet?.avatarUrl || '';
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              <div>
                <label className="block font-semibold text-[#1c1c18] dark:text-[#fcfbf7] mb-1 flex items-center justify-between">
                  <span>Cadet No. (Login ID) <span className="text-red-500">*</span></span>
                  {isProfileUpdateMode && (
                ...
                <input
                  type="text"
                  required={!isProfileUpdateMode}
                  disabled={isProfileUpdateMode}
                  placeholder="e.g. NGDC-M-101 / 24-105"
                  value={servingForm.cadetNo || editingCadet?.cadetNo || ''}
                  onChange={(e) => setServingForm({ ...servingForm, cadetNo: e.target.value })}
                  className={`w-full border px-3.5 py-2.5 rounded-xl font-mono uppercase font-bold outline-none ${
                    isProfileUpdateMode
                      ? 'bg-[#e5e1d8] dark:bg-[#12110f] border-amber-600/40 text-gray-500 dark:text-gray-400 cursor-not-allowed select-none'
                      : 'bg-[#fcf9f3] dark:bg-[#1e1d19] border-[#cdc6b3] dark:border-[#423e35] text-[#1c1c18] dark:text-[#fcfbf7]'
                  }`}
                />
                {isProfileUpdateMode && (
                  <p className="mt-1 text-[11px] text-amber-800 dark:text-amber-400 font-medium">
                    Cadet Number is permanent and cannot be modified.
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block font-semibold text-[#1c1c18] dark:text-[#fcfbf7] mb-1">
                  Login Password {isProfileUpdateMode ? <span className="text-xs font-normal text-gray-500 dark:text-gray-400">(Optional to change)</span> : <span className="text-red-500">*</span>}
                </label>
                <input
                  type="text"
                  required={!isProfileUpdateMode}
                  placeholder={isProfileUpdateMode ? "Leave blank to keep current password" : "Create Login Password"}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
              ...
              <div>
                ...
                <input
                  ...
                  value={exForm.cadetNo || editingCadet?.cadetNo || ''}
