...
export const FooterTab: React.FC = () => {
  ...
  useEffect(() => {
    if (!isDirty && footerConfig) {
      setForm(footerConfig);
    }
  }, [footerConfig, isDirty]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSaving(true);

    // Build updated socialLinks array so it stays in exact sync with individual social URL fields
    const updatedSocialLinks = [
      { id: 'soc-fb', platform: 'facebook', label: 'Official Facebook Page', url: form.facebookUrl || '' },
      { id: 'soc-yt', platform: 'youtube', label: 'YouTube Channel', url: form.youtubeUrl || '' },
      { id: 'soc-ig', platform: 'instagram', label: 'Instagram Profile', url: form.instagramUrl || '' },
      { id: 'soc-tw', platform: 'twitter', label: 'X (Twitter)', url: form.twitterUrl || '' },
    ];
