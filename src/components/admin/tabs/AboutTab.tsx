...
import {
  ...
  DEFAULT_BNCCO1_MESSAGE,
  DEFAULT_BNCCO2_MESSAGE,
  DEFAULT_PLATOON_COMMANDER_MESSAGE,
...
export const AboutTab: React.FC = () => {
  ...
  const [overviewForm, setOverviewForm] = useState(aboutOverview || DEFAULT_ABOUT_OVERVIEW);
  ...
  const [bncco1Form, setBncco1Form] = useState(bncco1Message || DEFAULT_BNCCO1_MESSAGE);

  // Local form state for BNCCO 2
  const [bncco2Form, setBncco2Form] = useState(bncco2Message || DEFAULT_BNCCO2_MESSAGE);

  // Local form state for Platoon Commander
  const [commanderForm, setCommanderForm] = useState(platoonCommanderMessage || DEFAULT_PLATOON_COMMANDER_MESSAGE);
  ...
  const handleResetOverview = async () => {
    if (confirm('Reset About Our Platoon narrative to factory default?')) {
      ...
      setIsOverviewDirty(false);
      setOverviewForm(DEFAULT_ABOUT_OVERVIEW);
  ...
  const handleResetBncco1 = async () => {
    if (confirm('Reset BNCCO 1 message to default?')) {
      ...
      setIsBncco1Dirty(false);
      setBncco1Form(DEFAULT_BNCCO1_MESSAGE);
  ...
  const handleResetBncco2 = async () => {
    if (confirm('Reset BNCCO 2 message to default?')) {
      ...
      setIsBncco2Dirty(false);
      setBncco2Form(DEFAULT_BNCCO2_MESSAGE);
  ...
  const handleResetCommander = async () => {
    if (confirm('Reset Platoon Commander message to default?')) {
      ...
      setIsCommanderDirty(false);
      setCommanderForm(DEFAULT_PLATOON_COMMANDER_MESSAGE);
