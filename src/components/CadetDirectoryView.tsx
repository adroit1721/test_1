...
import { fetchOfficerAccounts } from '../utils/apiClient';
import {
  Search,
  Filter,
  User,
...
import { CadetUserAccount, OfficerAccount } from '../types';
...
export const CadetDirectoryView: React.FC<CadetDirectoryViewProps> = ({ isCadetLoggedIn = false }) => {
  ...
  // Tab State: Cadets vs Officers
  const [activeTab, setActiveTab] = useState<'cadets' | 'officers'>('cadets');
  ...
  useEffect(() => {
    const loadOfficers = async () => {
      try {
        const accs = await fetchOfficerAccounts();
          ...
          setOfficers([
            {
              ...
              image: '/assets/puo_commander.jpg',
              status: 'active'
            },
            {
              id: 'off-2',
              ...
              appointmentRole: 'Vice Principal & Battalion Inspector',
              username: 'OFFICER_VP',
              phone: '+8801711000000',
  ...
  return (
    <div className="space-y-6">
      {/* Directory Tab Switcher: Cadets vs Officers */}
      <div className="flex items-center gap-2 border-b border-[#cdc6b3]/50 dark:border-[#38342c] pb-2">
        <button
          ...
          className={`px-5 py-2.5 rounded-2xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer flex items-center gap-2 ${
            activeTab === 'cadets'
              ? 'bg-[#6b5e10] text-white shadow-md'
              : 'bg-white dark:bg-[#1e1d19] text-[#695c4e] dark:text-[#aca596] border border-[#cdc6b3]/60 dark:border-[#423e35]'
          }`}
        >
          <Users className="w-4 h-4" />
          <span>Cadets Directory ({approvedCadets.length})</span>
        </button>
        ...
        >
          ...
          <span>Officers & Platoon Command ({officers.length})</span>
        </button>
      </div>
      ...
      {activeTab === 'officers' ? (
        ...
        <div className="space-y-6">
          <div className="bg-[#fcf9f3] dark:bg-[#1e1d19] border border-[#cdc6b3]/60 dark:border-[#423e35] p-5 sm:p-6 rounded-3xl space-y-2 shadow-xs">
            <div className="flex items-center gap-2 text-[#6b5e10] dark:text-[#eedc82]">
              <Shield className="w-5 h-5" />
              <h2 className="text-xl font-black text-[#1c1c18] dark:text-[#fcfbf7]">
                Platoon Command Officers Directory
              </h2>
            </div>
            <p className="text-xs text-[#695c4e] dark:text-[#aca596]">
              Verified profile directory of Platoon Commanders, Faculty Officers, and Battalion Inspectors of New Govt. Degree College BNCC Platoon.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {officers.map((officer) => {
              ...
              const safeImage = officer.picture || officer.image || officer.photoUrl;
              const appointmentRole = officer.appointmentRole || officer.role || 'Platoon Officer';

              return (
                <div
                  key={officer.id || officer.username}
                  className="bg-white dark:bg-[#1c1b17] border border-[#cdc6b3] dark:border-[#38342b] hover:border-[#6b5e10] dark:hover:border-[#eedc82] rounded-3xl p-6 shadow-sm space-y-4 relative overflow-hidden group transition-all"
                >
                  <div className="flex items-start gap-4">
                    <div className="relative">
                      <div className="w-20 h-20 rounded-2xl bg-[#eedc82]/30 border-2 border-[#6b5e10]/30 overflow-hidden flex items-center justify-center text-[#6b5e10] dark:text-[#eedc82] font-black text-2xl shrink-0 shadow-sm">
                        {safeImage ? (
                          <img
                            src={safeImage}
                            alt={safeName}
                      ...
                      <span className="absolute -bottom-1 -right-1 bg-[#6b5e10] text-[#eedc82] text-[9px] font-black px-2 py-0.5 rounded-full uppercase">
                        {officer.rank || 'OFFICER'}
                      </span>
                    </div>

                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-[#eedc82]/20 text-[#6b5e10] dark:text-[#eedc82] border border-[#6b5e10]/20">
                        <BadgeCheck className="w-3 h-3" />
                        <span>{appointmentRole}</span>
                      </div>
                      <h3 className="text-lg font-black text-[#1c1c18] dark:text-white truncate">
                        {safeName}
                      </h3>
                      <p className="text-xs text-[#695c4e] dark:text-[#aca596] font-semibold">
                        {officer.designation}
                      </p>
                    </div>
                  </div>

                  <div className="bg-[#fcfbf9] dark:bg-[#12110e] border border-[#cdc6b3]/50 dark:border-[#38342b] rounded-2xl p-4 text-xs space-y-2">
                    <div className="flex items-center gap-2 text-[#1c1c18] dark:text-white font-medium">
                      <Building2 className="w-4 h-4 text-[#6b5e10] shrink-0" />
                      <span>Institution: <strong>New Govt. Degree College, Rajshahi</strong></span>
                    </div>

                    {officer.phone && (
                      <div className="flex items-center gap-2 text-[#1c1c18] dark:text-white font-mono">
                        <Phone className="w-4 h-4 text-[#6b5e10] shrink-0" />
                        <span>Phone: <a href={`tel:${officer.phone}`} className="hover:underline font-bold text-[#6b5e10] dark:text-[#eedc82]">{officer.phone}</a></span>
                      </div>
                    )}

                    {officer.email && (
                      <div className="flex items-center gap-2 text-[#1c1c18] dark:text-white font-mono">
                        <Mail className="w-4 h-4 text-[#6b5e10] shrink-0" />
                        <span>Email: <a href={`mailto:${officer.email}`} className="hover:underline font-bold text-[#6b5e10] dark:text-[#eedc82]">{officer.email}</a></span>
                      </div>
                    )}
                  </div>
                </div>
