  ...
  return (
    <footer className="max-w-[1120px] mx-auto mt-14 sm:mt-18 mb-6 sm:mb-8 border border-[#cdc6b3]/50 dark:border-[#423e35] rounded-2xl md:rounded-3xl relative bg-transparent w-full pt-8 pb-6 px-5 sm:px-8 md:px-10 transition-colors">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-6">
        ...
        <div className="space-y-3.5">
          ...
          <ul className="flex flex-col space-y-2 text-xs sm:text-[13px]">
            <li>
              <a
                id="footer-link-officer-portal"
                href="/officer/login"
                onClick={(e) => {
                  e.preventDefault();
                  window.history.pushState({}, '', '/officer/login');
                  window.dispatchEvent(new PopStateEvent('popstate'));
                  window.scrollTo({ top: 0, behavior: 'smooth' });
                }}
                className="text-left text-[#555042] dark:text-[#9e9788] hover:text-[#6b5e10] dark:hover:text-[#eedc82] transition-colors cursor-pointer inline-flex items-center gap-1.5 font-medium"
              >
                <span className="font-bold text-[#6b5e10] dark:text-[#eedc82]">Officer & Teacher Portal</span>
                <span className="text-[10px] font-bold px-1.5 py-0.2 rounded bg-[#6b5e10]/15 text-[#6b5e10] dark:text-[#eedc82]">
                  Read-Only
                </span>
              </a>
            </li>
            ...
            {footerConfig.importantLinks && footerConfig.importantLinks.map((link) => {
              ...
              if (
                labelLower.includes('officer') ||
                labelLower.includes('teacher') ||
                labelLower.includes('college official') ||
                labelLower.includes('directorate') ||
                labelLower.includes('cadets corner') ||
                labelLower.includes('recruitment') ||
                labelLower.includes('training') ||
                labelLower.includes('notice board')
              ) {
                return null;
              }
              return (
      ...
      <div className="mt-8 pt-5 border-t border-[#cdc6b3]/40 dark:border-[#423e35] flex flex-col items-center justify-center text-center relative space-y-2">
        ...
        <p className="text-[11px] text-[#8c8474] dark:text-[#706a5e]">
        ...
        <div className="group/footerbottom min-h-[28px] flex items-center justify-center pt-1">
          <a
