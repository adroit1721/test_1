...
export const CadetsCornerView: React.FC<CadetsCornerProps> = ({
  ...
  const [idCardName, setIdCardName] = useState(currentCadet?.name || 'BNCC Cadet');
  const [idCardRank, setIdCardRank] = useState(currentCadet?.rank || 'Cadet (CDT)');
  const [idCardBlood, setIdCardBlood] = useState(currentCadet?.bloodGroup || 'B+');
  const [idCardBatch, setIdCardBatch] = useState(currentCadet?.batch || 'Batch 24');
  const [idCardNo, setIdCardNo] = useState(currentCadet?.cadetNo || 'NGDC-2024-001');

  // Sync ID card whenever logged-in cadet changes
  useEffect(() => {
    if (currentCadet) {
      setIdCardName(currentCadet?.name || 'BNCC Cadet');
      setIdCardRank(currentCadet?.rank || 'Cadet (CDT)');
      setIdCardBlood(currentCadet?.bloodGroup || 'B+');
      setIdCardBatch(currentCadet?.batch || 'Batch 24');
      setIdCardNo(currentCadet?.cadetNo || 'NGDC-2024-001');
    }
  }, [currentCadet]);
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (result.cadet) {
      const activeCadet: CadetProfile = {
        ...
        name: result.cadet.name || 'Cadet',
        rank: result.cadet.rank || 'Cadet (CDT)',
        platoon: `${result.cadet.category || 'BNCC Platoon'} (${result.cadet.section || 'Platoon HQ'})`,
