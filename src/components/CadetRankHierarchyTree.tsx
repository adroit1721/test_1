...
export const CadetRankHierarchyTree: React.FC = () => {
  ...
  const servingCadets = useMemo(() => {
    ...
    return cadetUsers.filter((c) => {
      ...
      if (status.includes('pending') || c.isApproved === false || (c as any).isApproved === 'false') {
        return false;
      }
      return true;
    });
  ...
  const bandCount = useMemo(() => {
    return servingCadets.filter((c) => {
      ...
      const plt = (c.platoon ? String(c.platoon) : '').toLowerCase().trim();
      const sec = (c.section ? String(c.section) : '').toLowerCase().trim();
      return cat.includes('band') || plt.includes('band') || sec.includes('band');
    }).length;
  }, [servingCadets]);
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#fcf9f3] dark:bg-[#1e1d19] p-2.5 sm:p-3 rounded-2xl border border-[#cdc6b3]/60 dark:border-[#38352d]">
        ...
        <div className="flex items-center gap-1.5 p-1 bg-[#f6f3ed] dark:bg-[#141311] rounded-xl border border-[#cdc6b3]/50 dark:border-[#38352d] w-full sm:w-auto">
          {(['Male Platoon', 'Female Platoon', 'Band Platoon'] as const).map((tab) => {
            ...
            const tabCount = tab === 'Male Platoon' ? maleCount : tab === 'Female Platoon' ? femaleCount : bandCount;
            return (
              <button
                key={tab}
                type="button"
                onClick={() => setActiveTab(tab)}
                className={`flex-1 sm:flex-none px-3.5 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${isActive
              ...
              >
                ...
                <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-bold ${
                  isActive ? 'bg-[#eedc82] text-[#1c1c18]' : 'bg-black/10 dark:bg-white/10 text-current'
                }`}>
                  {tabCount}
                </span>
              </button>
        ...
        <div className="flex items-center gap-2 text-xs w-full sm:w-auto justify-between sm:justify-end">
          <button
            ...
            disabled={isSyncing}
            className="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-[#eedc82]/30 hover:bg-[#eedc82] text-[#1c1c18] flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
            title="Refresh latest hierarchy from cloud"
          >
