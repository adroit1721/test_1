...
export async function uploadImageToCloudinary(
  ...
  // 1. Direct browser Cloudinary upload (Bypasses Render server bandwidth entirely)
  const config = getCloudinaryConfig();
  if (config.cloudName && config.uploadPreset) {
    try {
      ...
      console.warn('Direct Cloudinary upload failed, attempting server proxy fallback:', err);
    }
  }

  // 2. Server-side Cloudinary proxy fallback
  try {
    const base64Data = skipCompression
      ? await fileToDataUrl(file)
      : await compressAndConvertToDataUrl(file, 800, 800, 0.9);
    if (base64Data) {
    ...
    console.warn('[Cloudinary] Server proxy upload failed:', serverErr);
  }

  // 3. Safe fallback to browser-side data URL
  return skipCompression ? await fileToDataUrl(file) : await compressAndConvertToDataUrl(file, 600, 600, 0.85);
}
...
async function compressFileToPassportCanvas(file: File): Promise<File> {
...
export async function processPassportPhoto(
  ...
  // If File, first compress 15MB file down to 300x300 passport JPEG (~20KB) in client browser
  const optimizedFile = await compressFileToPassportCanvas(file);

  // Upload optimized file to Cloudinary
  try {
    const cUrl = await uploadImageToCloudinary(optimizedFile, 'cadet-passport-photos', true);
    if (cUrl && (cUrl.startsWith('http://') || cUrl.startsWith('https://'))) {
      return {
        url: cUrl,
        fileSizeKb: Math.round(optimizedFile.size / 1024),
        isCompliant: true,
      };
    }
  } catch (e) {
