...
export const AdminLogin: React.FC<AdminLoginProps> = ({
  ...
  const {
    loginOfficer,
    lockoutStatus = { isLocked: false, remainingSeconds: 0 },
    headerLeftLogoUrl,
    headerRightLogoUrl,
    headerTitle,
    headerSubtitle,
  ...
  const [countdown, setCountdown] = useState<number>(lockoutStatus?.remainingSeconds || 0);

  useEffect(() => {
    setCountdown(lockoutStatus?.remainingSeconds || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isMasterAdmin = (cleanUser.toLowerCase() === 'ngdc_bncc_1979' && cleanPass === 'Ngdc$BNCC$1979') ||
